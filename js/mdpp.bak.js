...
mdppManager.prototype.init = function(){
	this.curr_state_id = dict_doc_type['none'];
...
mdppManager.prototype.exec_mdpp_cmd = function(mdpp_next_statement_class_id, mdpp_type_id, mdpp_next_statement){
	if (this.curr_state_id == dict_doc_class['none']){
		switch(mdpp_next_statement_class_id){
			case dict_doc_class['markdown']:
				this.curr_state_id = dict_doc_class['markdown'];
				this.list_state.push(mdpp_next_statement);
			case dict_doc_class['mdpp']:
				this.curr_state_id = dict_doc_class['mdpp'];
				
			default:
				this.curr_state_id = dict_doc_class['none'];
		}
...
mdppManager.prototype.str_mdpp2list_mdpp_object= function (str_mdpp) {	
	...
	mdpp_next_statement_class_id = dict_doc_class['none'];
	var cnt = 0;
	for( each_mdpp_statement_id in list_mdpp_statement){
		console.log('state:'+mdpp_state.curr_state);
		console.log('input[' + cnt +']:'+ list_mdpp_statement[each_mdpp_statement_id]);
	
		
		//console.log(list_mdpp_statement[each_mdpp_statement]);
		
		for ( each_pattern_id in dict_mdpp_pattern) {				
			if ( dict_mdpp_pattern[each_pattern_id].test(list_mdpp_statement[each_mdpp_statement_id])) {
					//console.log(each_pattern);
					//tmp_html_content = "";					
				...
				mdpp_next_statement_class_id = dict_doc_class['mdpp'];				
				break;
			}
			
		}
		
		mdpp_state.exec(mdpp_next_statement_class_id, each_pattern_id, list_mdpp_statement[each_mdpp_statement_id]);
...
function mdpp2ListMdppObject_bak(str_mdpp) {
...
mdppManager.prototype mdpp2ListDiv = function (input_content) {
	...
	ListMdppObject = str_mdpp2list_mdpp_object(str_mdpp)
